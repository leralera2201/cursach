...
<h3 class="text-center pt-4 pb-2">Tour List</h3>
<div class="container text-center">
    ...
    <dix>
        <a href="/web/tour/sorted-list" class="btn btn-outline-secondary mb-4">Sort table by name</a>
        <a href="/web/tour/sorted-list-by-date" class="btn btn-outline-secondary mb-4">Sort table by date</a>
        <a href="/web/tour/list" class="btn btn-outline-secondary mb-4">Undo sort</a>
    </dix>
    ...
    <table class="table table-hover pt-5">
        ...
        <#list tours as tour>
            <tr>
                ...
                <td>${tour.instructor.nameandSurname}</td>
                <td>${tour.group.group_name}</td>
                <td>${tour.tour_full_time}</td>
                <td>${tour.payment}</td>
                <td>${tour.route}</td>
                <td>${tour.route_length}</td>
                <td>${tour.tour_date}</td>
                <td>${tour.tour_type.type_name}</td>
                <td>${tour.category.category_name}</td>
                <td><a href="delete/${tour.id}">
                        <button class="btn btn-outline-secondary">Delete</button>
                    </a></td>
                <td><a href="edit/${tour.id}"><button class="btn btn-outline-secondary">Edit</button></a></td>
            </tr>
        </#list>
        </tbody>
    </table>
