...
<h3 class="text-center pt-4 pb-2">Captain List</h3>
<div class="container text-center">
    ...
    <div>
        <a href="/web/captain/sorted-list" class="btn btn-outline-secondary mb-4">Sort table by name</a>
        <a href="/web/captain/list" class="btn btn-outline-secondary mb-4">Undo sort</a>
    </div>
    ...
    <table class="table table-hover pt-5">
        ...
        <#list captains as captain>
            <tr>
                <td>${captain?index + 1}</td>
                <td>${captain.surname}</td>
                <td>${captain.name}</td>
                <td>${captain.patronic}</td>
                <#if captain.sex>
                    <td>Man</td>
                <#else>
                    <td>Woman</td>
                ...
                <td>${captain.birthday}</td>
                <td>${captain.started_work_year}</td>
                <td>${captain.salary}</td>
                <td>${captain.phone}</td>
                <td>${captain.email}</td>
                <td><a href="delete/${captain.id}">
                        <button class="btn btn-outline-secondary">Delete</button>
                    </a></td>
                <td><a href="edit/${captain.id}"><button class="btn btn-outline-secondary">Edit</button></a></td>
            </tr>
        </#list>
        </tbody>
    </table>
