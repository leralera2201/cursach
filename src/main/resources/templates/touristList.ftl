...
<h3 class="text-center pt-4 pb-2">Tourist List</h3>
<div class="container text-center">
    ...
    <div>
        <a href="/web/tourist/sorted-list" class="btn btn-outline-secondary mb-4">Sort table by name</a>
        <a href="/web/tourist/list" class="btn btn-outline-secondary mb-4 ml-2">Undo sort</a>
    </div>
    ...
    <table class="table table-hover pt-5">
        ...
        <tr class="table-secondary">
            ...
            <th scope="col">Patronic</th>
            <th scope="col">Sex</th>
            <th scope="col">Birthday</th>
            <th scope="col">Health</th>
            <th scope="col">HealhProblems</th>
        ...
        <#list tourists as tourist>
            <tr>
                <td>${tourist?index + 1}</td>
                <td>${tourist.surname}</td>
                <td>${tourist.name}</td>
                <td>${tourist.patronic}</td>
                <#if tourist.sex>
                    <td>Man</td>
                <#else>
                    <td>Woman</td>
                ...
                <td>${tourist.physical_data}</td>
                <td>${tourist.negative_features}</td>
                <td>${tourist.phone}</td>
                <td>${tourist.email}</td>
                <#if tourist.max_category?has_content>
                    <td>${tourist.max_category.categoryName}</td>
                <#else>
                    <td>-</td>
                </#if>

                <td><a href="delete/${tourist.id}">
                        <button class="btn btn-outline-secondary">Delete</button>
                    </a></td>
                <td><a href="edit/${tourist.id}"><button class="btn btn-outline-secondary">Edit</button></a></td>
